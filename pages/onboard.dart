  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(children: [
              ...
              itemBuilder: (_, i) {
                return Padding(padding: const EdgeInsets.all(20),
                  child: Column(children: [
                    Image.asset(contents[i].image, height: 450,
                      width: MediaQuery
                          .of(context)
                          .size
                          .width / 1.5, fit: BoxFit.fill,),
        ...
        GestureDetector(
          onTap: () {
            //helps to shows the current page
            log('Current Index: $currentIndex, length: ${contents.length}',name:"TESTING");
            if (currentIndex == contents.length - 1) {
              Navigator.pushReplacement(context,
                  MaterialPageRoute(builder: (context) => const Signup()));
            }
            _controller.nextPage(duration: const Duration(
                milliseconds: 100), curve: Curves.bounceIn);
            setState(() {
              currentIndex++;// help on the increment of the next page
            });
          },
